    ...
    const handleSelectOption = (event) => {
     ...
     function handleSubmit(event) {
        ...
        if (
            ...
            price === "" ||
            (selectedOption === "") ||
            (selectedOption === "book" && weight === "") ||
            (selectedOption === "dvd" && size === "") ||
            (selectedOption === "furniture" && (height === "" || width === "" || length === ""))
          ) {
            setMensagem("Please, submit required data");
          } else {
            console.log([sku, name, price, selectedOption, size, weight, height, width, length]);
          }

          if (!sku.match(/^[^$/&*%#@!'"+{}[\]?\\:;,.|]+$ /)) {
            alert("Por favor, forneça o SKU como caracteres alfanuméricos.");
            return;
          }
